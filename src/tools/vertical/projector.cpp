...
void OBSProjector::mousePressEvent(QMouseEvent *event)
{
	...
	if (event->button() == Qt::RightButton) {
		...
		if (GetMonitor() > -1) {
			...
			popup.addAction(
				QString::fromUtf8(obs_frontend_get_locale_string("ResizeProjectorWindowToContent")),
				this, SLOT(ResizeToContent()));
		}

		QAction *alwaysOnTopButton = new QAction(
			QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.View.AlwaysOnTop")), this);
		...
		popup.addAction(QString::fromUtf8(obs_frontend_get_locale_string("Close")), this,
...
void OBSProjector::UpdateProjectorTitle(QString name)
{
	...
	QString title =
		QString::fromUtf8(obs_frontend_get_locale_string(window ? "PreviewWindow" : "PreviewProjector"));
