...
void SourceTreeItem::ExitEditModeInternal(bool save)
{
	...
	obs_canvas_t *canvas = (obs_source_get_output_flags(source) & OBS_SOURCE_REQUIRES_CANVAS)
				       ? obs_source_get_canvas(source)
				       : nullptr;
...
QString SourceTreeModel::GetNewGroupName()
{
	...
	for (;;) {
		OBSSourceAutoRelease group =
			obs_canvas_get_source_by_name(st->canvasDock->canvas, name.toUtf8().constData());
		if (!group)
			break;
...
void SourceTreeModel::GroupSelectedItems(QModelIndexList &indices)
{
	...
	obs_sceneitem_t *item =
		obs_scene_insert_group(scene, name.toUtf8().constData(), item_order.data(), item_order.size());
	if (!item) {
		return;
	}
...
void SourceTree::UpdateNoSourcesMessage()
{
	...
	textNoSources.setText(
		QString::fromUtf8(obs_frontend_get_locale_string("NoSources.Label")).replace("\n", "<br/>"));
