...
void CanvasScenesDock::ShowScenesContextMenu(QListWidgetItem *widget_item)
{
	...
	orderMenu->addAction(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Order.MoveToBottom")),
	...
	for (auto t : canvasDock->transitions) {
		...
		connect(a2, &QAction::triggered, [this, scene_name, a2] {
			OBSSourceAutoRelease source =
				obs_canvas_get_source_by_name(canvasDock->canvas, scene_name.c_str());
			OBSDataAutoRelease ps = obs_source_get_private_settings(source);
	...
	a = menu.addAction(QString::fromUtf8(obs_frontend_get_locale_string("ShowInMultiview")), [this, scene_name](
													 bool checked) {
		OBSSourceAutoRelease source = obs_canvas_get_source_by_name(canvasDock->canvas, scene_name.c_str());
		OBSDataAutoRelease ps = obs_source_get_private_settings(source);
...
CanvasScenesDock::CanvasScenesDock(CanvasDock *canvas_dock, QWidget *parent) : QFrame(parent), canvasDock(canvas_dock)
{
	...
	connect(renameAction, &QAction::triggered, [this]() {
		...
		obs_source_t *source =
			obs_canvas_get_source_by_name(canvasDock->canvas, item->text().toUtf8().constData());
		if (!source) {
			return;
		}
	...
	a = toolbar->addAction(QIcon(":/res/images/filter.svg"),
			       QString::fromUtf8(obs_frontend_get_locale_string("SceneFilters")), [this] {
				       ...
				       auto s = obs_canvas_get_source_by_name(canvasDock->canvas,
