...
static void save_canvas()
{
	...
	char *path = obs_module_config_path(VERTICAL_CONFIG_FILE);
	if (!path) {
		return;
	}
...
void vendor_request_save_replay(obs_data_t *request_data, obs_data_t *response_data, void *p)
{
	...
	for (const auto &it : canvas_docks) {
		...
		QMetaObject::invokeMethod(it, "ReplayButtonClicked",
					  Q_ARG(QString,
						QString::fromUtf8(obs_data_get_string(request_data, "filename"))));
		obs_data_set_bool(response_data, "success", true);
		return;
	}
	obs_data_set_bool(response_data, "success", false);
...
void vendor_request_update_stream_server(obs_data_t *request_data, obs_data_t *response_data, void *)
{
	...
	for (const auto &it : canvas_docks) {
		...
		QMetaObject::invokeMethod(it, "updateStreamServer",
...
/* First run of the port: take over the original plugin's settings so
 * canvases, outputs and hotkeys carry over. */
static void vertical_canvas_migrate_config(void)
{
	char *ours = obs_module_config_path(VERTICAL_CONFIG_FILE);
	if (ours && !os_file_exists(ours)) {
		char *dir = obs_module_config_path("");
		os_mkdirs(dir);
		bfree(dir);
		/* <config>/plugin_config/meketreve-obs-essentials -> .../vertical-canvas */
		std::string theirs = ours;
		const auto pos = theirs.rfind("meketreve-obs-essentials");
		if (pos != std::string::npos) {
			theirs.replace(pos, strlen("meketreve-obs-essentials"), "vertical-canvas");
			theirs.replace(theirs.rfind(VERTICAL_CONFIG_FILE), strlen(VERTICAL_CONFIG_FILE), "config.json");
			if (os_file_exists(theirs.c_str()) && os_copyfile(theirs.c_str(), ours) == 0)
				blog(LOG_INFO, "[Vertical Canvas] imported settings from %s", theirs.c_str());
		}
	}
	bfree(ours);
}

/* Meketreve: the three module entry points below became plain functions
 * called from the toolkit's own obs_module_post_load/unload. */
static bool vertical_canvas_active = false;

static bool vertical_canvas_register_sources(void)
{
	blog(LOG_INFO, "[Vertical Canvas] loaded version %s (Meketreve OBS Essentials port)", PROJECT_VERSION);
...
extern "C" void vertical_canvas_post_load(void)
{
	/* The original plugin (or Aitum Stream Suite, which contains it) wins:
	 * running both would register the same sources and docks twice. */
	if (obs_get_module("vertical-canvas") || obs_get_module("aitum-stream-suite")) {
		blog(LOG_INFO, "[Vertical Canvas] Aitum's plugin is loaded, the Meketreve copy stays off");
		return;
	}
	vertical_canvas_register_sources();
	vertical_canvas_active = true;
	vertical_canvas_migrate_config();

	const auto path = obs_module_config_path(VERTICAL_CONFIG_FILE);
	obs_data_t *config = obs_data_create_from_json_file_safe(path, "bak");
	bfree(path);
	if (!config) {
		config = obs_data_create();
	...
	obs_websocket_vendor_register_request(vendor, "toggle_streaming", vendor_request_invoke,
	...
	obs_websocket_vendor_register_request(vendor, "toggle_recording", vendor_request_invoke,
					      (void *)"RecordButtonClicked");
	obs_websocket_vendor_register_request(vendor, "start_backtrack", vendor_request_invoke,
					      (void *)"StartReplayBuffer");
	obs_websocket_vendor_register_request(vendor, "stop_backtrack", vendor_request_invoke,
					      (void *)"StopReplayBuffer");
	obs_websocket_vendor_register_request(vendor, "save_backtrack", vendor_request_save_replay, nullptr);
	obs_websocket_vendor_register_request(vendor, "start_virtual_camera", vendor_request_invoke,
					      (void *)"StartVirtualCam");
	obs_websocket_vendor_register_request(vendor, "stop_virtual_camera", vendor_request_invoke,
					      (void *)"StopVirtualCam");
	obs_websocket_vendor_register_request(vendor, "update_stream_key", vendor_request_update_stream_key, nullptr);
	obs_websocket_vendor_register_request(vendor, "update_stream_server", vendor_request_update_stream_server,
					      nullptr);
...
extern "C" void vertical_canvas_unload(void)
{
	if (!vertical_canvas_active)
		return;
...
void CanvasDock::CheckReplayBuffer(bool start)
{
	...
	bool active = obs_frontend_streaming_active() || obs_frontend_recording_active() ||
		      obs_frontend_replay_buffer_active() || (recordOutput && obs_output_active(recordOutput));
	  ...
	  eventFilter(BuildEventFilter())
{
	...
	if (streamOutputs.empty() && strlen(obs_data_get_string(settings, "stream_server"))) {
		...
		ss.service = obs_service_create(whip ? "whip_custom" : "rtmp_custom",
						"vertical_canvas_stream_service_0", nullptr, nullptr);
	...
	const auto transitionsTitle =
		title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.SceneTransitions"));
	...
	auto enablePreviewButton = new QPushButton(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.PreviewConextMenu.Enable")));
	...
	connect(&recordDurationTimer, &QTimer::timeout, [this] {
		if (obs_output_active(recordOutput)) {
			...
			auto t = QTime::fromMSecsSinceStartOfDay(
				(int)util_mul_div64(totalFrames, frameTimeNs, 1000000ULL));
	...
	aitumButton->setStyleSheet(
		QString::fromUtf8("QPushButton{border-top-left-radius: 0; border-bottom-left-radius: 0;}"));
	...
	virtual_cam_hotkey = obs_hotkey_pair_register_frontend(
		...
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.StartVirtualCam")))
			.toUtf8()
			.constData(),
		"VerticalCanvasDockStopVirtualCam",
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.StopVirtualCam")))
			.toUtf8()
			.constData(),
	...
	record_hotkey = obs_hotkey_pair_register_frontend(
		...
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.StartRecording")))
			.toUtf8()
			.constData(),
		"VerticalCanvasDockStopRecording",
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.StopRecording")))
			.toUtf8()
			.constData(),
	...
	stream_hotkey = obs_hotkey_pair_register_frontend(
		...
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.StartStreaming")))
			.toUtf8()
			.constData(),
		"VerticalCanvasDockStopStreaming",
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.StopStreaming")))
			.toUtf8()
			.constData(),
	...
	pause_hotkey = obs_hotkey_pair_register_frontend(
		...
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.PauseRecording")))
			.toUtf8()
			.constData(),
	...
	preview_hotkey = obs_hotkey_pair_register_frontend(
		...
		(title + " " + QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.Preview.Disable")))
			.toUtf8()
			.constData(),
...
void CanvasDock::DrawPreview(void *data, uint32_t cx, uint32_t cy)
{
	...
	if (window->selectionBox) {
		...
		window->DrawSelectionBox(window->startPos.x * scale, window->startPos.y * scale,
					 window->mousePos.x * scale, window->mousePos.y * scale, window->rectFill);
	}
	gs_load_vertexbuffer(nullptr);
...
void CanvasDock::DrawSpacingHelpers(obs_scene_t *s, float x, float y, float cx, float cy, float scale, float sourceX,
				    float sourceY)
{
	UNUSED_PARAMETER(x);
	UNUSED_PARAMETER(y);
	...
	if (parentGroup) {
		...
		matrix4_rotate_aa4f(&boxTransform, &boxTransform, 0.0f, 0.0f, 1.0f,
...
static bool CenterAlignSelectedItems(obs_scene_t *scene, obs_sceneitem_t *item, void *param)
{
	...
	vec2_set(&itemInfo.bounds, float(obs_source_get_base_width(scene_source)),
...
void CanvasDock::AddSceneItemMenuItems(QMenu *popup, OBSSceneItem sceneItem)
{
	...
	popup->addAction(QString::fromUtf8(obs_frontend_get_locale_string("Rename")), [this, sceneItem] {
		...
		do {
			...
			s = canvas ? obs_canvas_get_source_by_name(canvas, name.c_str())
	...
	orderMenu->addAction(QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Order.MoveUp")),
			     this, [sceneItem] { obs_sceneitem_set_order(sceneItem, OBS_ORDER_MOVE_UP); });
	orderMenu->addAction(QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Order.MoveDown")),
			     this, [sceneItem] { obs_sceneitem_set_order(sceneItem, OBS_ORDER_MOVE_DOWN); });
	orderMenu->addSeparator();
	orderMenu->addAction(QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Order.MoveToTop")),
			     this, [sceneItem] { obs_sceneitem_set_order(sceneItem, OBS_ORDER_MOVE_TOP); });
	orderMenu->addAction(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Order.MoveToBottom")), this,
		[sceneItem] { obs_sceneitem_set_order(sceneItem, OBS_ORDER_MOVE_BOTTOM); });

	auto transformMenu =
		popup->addMenu(QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Transform")));
	transformMenu->addAction(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Transform.EditTransform")),
		[this, sceneItem] {
			...
			QMetaObject::invokeMethod(transformDialog,
						  obs_get_version() >= MAKE_SEMANTIC_VERSION(32, 1, 0) ? "setItemQt"
												       : "SetItemQt",
	...
	transformMenu->addAction(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.Transform.HorizontalCenter")),
...
bool CanvasDock::HandleMouseReleaseEvent(QMouseEvent *event)
{
	...
	if (!mouseDown && event->button() == Qt::RightButton) {
		QMenu popup(this);
		QAction *a = popup.addAction(
			QString::fromUtf8(obs_frontend_get_locale_string("Basic.Main.Preview.Disable")), [this] {
		...
		auto projectorMenu =
			popup.addMenu(QString::fromUtf8(obs_frontend_get_locale_string("PreviewProjector")));
		...
		a = popup.addAction(
			QString::fromUtf8(obs_frontend_get_locale_string("Basic.MainMenu.Edit.LockPreview")), this,
			[this] { locked = !locked; });
		a->setCheckable(true);
		a->setChecked(locked);
...
vec2 CanvasDock::GetMouseEventPos(QMouseEvent *event)
{
	...
	vec2 pos;
	vec2_set(&pos, ((float)qtPos.x() - (float)x / pixelRatio) / scale,
		 ((float)qtPos.y() - (float)y / pixelRatio) / scale);
...
void CanvasDock::CropItem(const vec2 &pos)
{
	...
	vec2_set(&max_br, stretchItemSize.x + (float)crop.right * scale.x,
		 stretchItemSize.y + (float)crop.bottom * scale.y);
...
bool CanvasDock::add_sources_of_type_to_menu(void *param, obs_source_t *source)
{
	...
	if (strcmp(obs_source_get_unversioned_id(source), id) == 0) {
		...
		connect(
			na, &QAction::triggered, cd, [cd, source] { cd->AddSourceToScene(source); },
			Qt::QueuedConnection);
...
void CanvasDock::AddSourceTypeToMenu(QMenu *popup, const char *source_type, const char *name)
{
	...
	QObject::connect(menu, &QMenu::aboutToShow,
			 [this, menu, source_type] { LoadSourceTypeMenu(menu, source_type); });
...
void CanvasDock::AddSourceFromAction()
{
	...
	if (id && *id && strlen(id)) {
		...
		if (obs_get_source_output_flags(id) & OBS_SOURCE_REQUIRES_CANVAS) {
			...
			created_source =
				obs_scene_get_source(obs_canvas_scene_create(canvas, text.toUtf8().constData()));
		} else {
...
void CanvasDock::StartVirtualCam()
{
	...
	if (virtual_cam_mode == VIRTUAL_CAMERA_VERTICAL) {
		...
		if (!multiCanvasSource) {
			multiCanvasSource = obs_source_create_private("vertical_multi_canvas_source",
		...
		if (!multiCanvasVideo) {
			...
			obs_video_info ovi;
			if (!obs_canvas_get_video_info(multiCanvas, &ovi) || ovi.base_width != w ||
			    ovi.base_height != h || ovi.output_width != w || ovi.output_height != h) {
				obs_get_video_info(&ovi);
				ovi.base_width = w;
				ovi.base_height = h;
				ovi.output_width = w;
...
void CanvasDock::StartRecord()
{
	...
	if (record_advanced_settings) {
		...
		if (!recordOutput || strcmp(obs_output_get_id(recordOutput), obs_output_get_id(output)) != 0) {
			obs_output_release(recordOutput);
			recordOutput = obs_output_create(obs_output_get_id(output), "vertical_canvas_record", nullptr,
							 nullptr);
		}
	...
	if (recordPath.empty() && (!dir || !strlen(dir))) {
		if (isVisible()) {
			QMessageBox::warning(this,
					     QString::fromUtf8(obs_frontend_get_locale_string("Output.BadPath.Title")),
		...
		for (size_t i = 0; i < MAX_AUDIO_MIXES; i++) {
			...
			if (aef) {
				...
				if (!aet) {
					...
					name += "_vertical";
					aet = obs_audio_encoder_create(obs_encoder_get_id(aef), name.c_str(), nullptr,
								       i, nullptr);
					obs_encoder_set_audio(aet, obs_get_audio());
				}
	...
	if (stream_advanced_settings) {
		...
		if (strcmp(mode, "Advanced") == 0) {
			...
			obs_data_set_string(video_settings,
					    (strcmp(presetType, "NVENCPreset2") == 0) ? "preset2" : "preset", preset);
...
obs_encoder_t *CanvasDock::GetRecordVideoEncoder()
{
	...
	if (!video_encoder) {
		video_encoder =
			obs_video_encoder_create(enc_id, "vertical_canvas_record_video_encoder", nullptr, nullptr);
	}
...
void CanvasDock::StartStreamOutput(std::vector<StreamServer>::iterator it)
{
	...
	if (it->settings && obs_data_get_bool(it->settings, "advanced") && obs_get_module("aitum-multistream")) {
		blog(LOG_INFO, "[Vertical Canvas] Start output '%s' with multistream advanced settings",
		     it->name.c_str());
		...
		if (!venc_name || venc_name[0] == '\0') {
			...
			if (scale) {
				...
				auto func2 = (void (*)(obs_encoder_t *, obs_scale_type))os_dlsym(
					handle, "obs_encoder_set_gpu_scale_type");
				if (func2) {
...
void CanvasDock::CreateStreamOutput(std::vector<StreamServer>::iterator it)
{
	...
	if (handle) {
		...
		if (!type_func) {
			type_func = (const char *(*)(obs_service_t *))os_dlsym(handle,
		...
		if (!type) {
			...
			if (url_func) {
				...
				auto info_func = (const char *(*)(obs_service_t *, uint32_t))os_dlsym(
					handle, "obs_service_get_connect_info");
				if (info_func) {
...
obs_encoder_t *CanvasDock::GetStreamAudioEncoder()
{
		...
		if (record_advanced_settings) {
			...
			if (!audioBitrate) {
				...
				const int advAudioBitrate =
					(int)config_get_uint(config, "AdvOut", trackNames[streamTrack - 1]);
	...
	if (!audio_encoder) {
		audio_encoder = obs_audio_encoder_create("ffmpeg_aac", "vertical_canvas_audio_encoder", audio_settings,
							 mix_idx, nullptr);
...
void CanvasDock::StartStream()
{
	...
	for (auto it = streamOutputs.begin(); it != streamOutputs.end(); ++it) {
		...
		if (it->settings && obs_data_get_bool(it->settings, "advanced") &&
		    obs_get_module("aitum-multistream")) {
			blog(LOG_INFO, "[Vertical Canvas] Start output '%s' with multistream advanced settings",
			     it->name.c_str());
			auto venc_name = obs_data_get_string(it->settings, "video_encoder");
			if (!venc_name || venc_name[0] == '\0') {
				...
				std::string video_encoder_name = "vertical_canvas_video_encoder_";
				video_encoder_name += it->name;
				auto venc = obs_video_encoder_create(venc_name, video_encoder_name.c_str(), ves_apply,
								     nullptr);
...
#endif
				if (divisor > 1) {
					auto func = (bool (*)(obs_encoder_t *, uint32_t))os_dlsym(
						handle, "obs_encoder_set_frame_rate_divisor");
					if (func) {
						func(venc, (uint32_t)divisor);
					}
					//obs_encoder_set_frame_rate_divisor(venc, (uint32_t)divisor);
				...
				if (scale) {
					...
					if (func2) {
						func2(venc,
						      (obs_scale_type)obs_data_get_int(it->settings, "scale_type"));
					}
					//obs_encoder_set_gpu_scale_type(venc, (obs_scale_type)obs_data_get_int(it->settings, "scale_type"));
				}
				if (handle) {
			...
			if (!aenc_name || aenc_name[0] == '\0') {
				...
				auto aenc = obs_audio_encoder_create(aenc_name, audio_encoder_name.c_str(), aes_apply,
								     obs_data_get_int(it->settings, "audio_track"),
								     nullptr);
	...
	for (auto it = streamOutputs.begin(); it != streamOutputs.end(); ++it) {
		...
		if (config) {
			...
			obs_data_set_string(output_settings, "ip_family",
					    config_get_string(config, "Output", "IPFamily"));
			obs_output_update(it->output, output_settings);
		}
		it->stopping = false;
		if (obs_output_start(it->output)) {
				...
				obs_hotkey_register_source(
					...
					[](void *data, obs_hotkey_id, obs_hotkey_t *key, bool pressed) {
						...
						auto potential_source =
							(obs_weak_source_t *)obs_hotkey_get_registerer(key);
						OBSSourceAutoRelease source =
							obs_weak_source_get_source(potential_source);
						if (source) {
							auto sn = QString::fromUtf8(obs_source_get_name(source));
							QMetaObject::invokeMethod(p, "SwitchScene", Q_ARG(QString, sn),
										  Q_ARG(bool, true));
...
void CanvasDock::SwitchScene(const QString &scene_name, bool transition)
{
	...
	auto fs = scene_name.isEmpty() ? nullptr
				       : obs_canvas_get_scene_by_name(canvas, scene_name.toUtf8().constData());
		...
		if (oldSource) {
			...
			if (ost == OBS_SOURCE_TYPE_TRANSITION) {
				...
				if (SwapTransition(override_transition)) {
					...
					signal_handler_connect(handler, "transition_stop", transition_override_stop,
							       this);
				}
				int duration = 0;
...
void CanvasDock::HandleRecordError(int code, QString last_error)
{
	...
	if (code == OBS_OUTPUT_UNSUPPORTED && isVisible()) {
		QMessageBox::critical(
			this, QString::fromUtf8(obs_frontend_get_locale_string("Output.RecordFail.Title")),
		...
		QString msg =
			last_error.isEmpty()
				...
				: QString::fromUtf8(
					  obs_frontend_get_locale_string("Output.RecordError.EncodeErrorMsg.LastError"))
					  .arg(last_error);
		QMessageBox::warning(
			this, QString::fromUtf8(obs_frontend_get_locale_string("Output.RecordError.Title")), msg);

	} else if (code == OBS_OUTPUT_NO_SPACE && isVisible()) {
		QMessageBox::warning(this,
				     QString::fromUtf8(obs_frontend_get_locale_string("Output.RecordNoSpace.Title")),
				     QString::fromUtf8(obs_frontend_get_locale_string("Output.RecordNoSpace.Msg")));

	} else if (code != OBS_OUTPUT_SUCCESS && isVisible()) {
		QMessageBox::critical(this,
				      QString::fromUtf8(obs_frontend_get_locale_string("Output.RecordError.Title")),
	...
	if (encode_error) {
		QString msg =
			last_error.isEmpty()
				...
				: QString::fromUtf8(
					  obs_frontend_get_locale_string("Output.StreamEncodeError.Msg.LastError"))
					  .arg(last_error);
		QMessageBox::information(
			this, QString::fromUtf8(obs_frontend_get_locale_string("Output.StreamEncodeError.Title")), msg);

	} else if (code != OBS_OUTPUT_SUCCESS && isVisible()) {
		QMessageBox::information(
			this, QString::fromUtf8(obs_frontend_get_locale_string("Output.ConnectFail.Title")),
...
QMenu *CanvasDock::CreateVisibilityTransitionMenu(bool visible, obs_sceneitem_t *si)
{
	QMenu *menu = new QMenu(
		QString::fromUtf8(obs_frontend_get_locale_string(visible ? "ShowTransition" : "HideTransition")));
...
bool CanvasDock::LoadStreamOutputs(obs_data_array_t *outputs)
{
	...
	for (size_t i = 0; i < count; i++) {
		...
		bool whip = strstr(ss.stream_server.c_str(), "whip") != nullptr;
		ss.service = obs_service_create(whip ? "whip_custom" : "rtmp_custom", service_name.c_str(), nullptr,
						nullptr);
		ss.settings = item;
		streamOutputs.push_back(ss);
	}
	...
	if (saving) {
		if (window->scenesDock) {
			...
			for (int row = 0; row < c; row++) {
				...
				auto scene =
					obs_canvas_get_source_by_name(window->canvas, scene_name.toUtf8().constData());
				if (scene) {
					auto settings = obs_source_get_settings(scene);
					obs_data_set_int(settings, "order", row);
					obs_data_set_bool(settings, "canvas_active",
							  scene_name == window->currentSceneName);
					obs_data_release(settings);
					obs_source_release(scene);
				}
			}
...
void CanvasDock::AskUpdate()
{
	...
	auto update = mb.addButton(QString::fromUtf8(obs_frontend_get_locale_string("Updater.UpdateNow")),
				   QMessageBox::YesRole);
	auto remind = mb.addButton(QString::fromUtf8(obs_frontend_get_locale_string("Updater.RemindMeLater")),
				   QMessageBox::RejectRole);
	auto skip =
		mb.addButton(QString::fromUtf8(obs_frontend_get_locale_string("Updater.Skip")), QMessageBox::NoRole);
