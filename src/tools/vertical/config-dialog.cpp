	  ...
	  canvasDock(canvas_dock)
{
	...
	virtualCameraMode->addItem(QString::fromUtf8(obs_module_text("VirtualCameraModeMain")),
				   QVariant(VIRTUAL_CAMERA_MAIN));
	virtualCameraMode->addItem(QString::fromUtf8(obs_module_text("VirtualCameraModeBoth")),
				   QVariant(VIRTUAL_CAMERA_BOTH));

	generalLayout->addRow(QString::fromUtf8(obs_frontend_get_locale_string("Basic.VCam.VirtualCamera")),
			      virtualCameraMode);
	...
	connect(button, &QPushButton::clicked, [this] {
		const QString dir = QFileDialog::getExistingDirectory(
			this, QString::fromUtf8(obs_module_text("BacktrackPath")), backtrackPath->text(),
	...
	auto streamDelayDurationLabel = new QLabel(
	...
	streamDelayPreserve = new QCheckBox(
	...
	auto streamingAdvancedGroup =
		new QGroupBox(QString::fromUtf8(obs_frontend_get_locale_string("Basic.Settings.Advanced")));
	...
	streamingAdvancedLayout->addRow(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.Settings.Output.Adv.AudioTrack")), audioTracks);
	...
	while (obs_enum_encoder_types(idx++, &type)) {
		...
		if (astrcmpi(codec, "h264") != 0 && astrcmpi(codec, "hevc") != 0 && astrcmpi(codec, "av1") != 0)
			continue;
		streamingEncoder->addItem(QString::fromUtf8(obs_encoder_get_display_name(type)),
	...
	connect(streamingEncoder, &QComboBox::currentIndexChanged, [streamingAdvancedLayout, this] {
		...
		while (property) {
			AddProperty(property, stream_encoder_settings, streamingAdvancedLayout,
	...
	streamingAdvancedLayout->addRow(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.Settings.Output.Encoder.Video")),
		streamingEncoder);
	...
	if (canvasDock->disable_stream_settings) {
		auto multistream_warning_widget =
			new QGroupBox(QString::fromUtf8(obs_frontend_get_locale_string("Warning")));
	...
	connect(button, &QPushButton::clicked, [this] {
		const QString dir = QFileDialog::getExistingDirectory(
			this,
			QString::fromUtf8(obs_frontend_get_locale_string("Basic.Settings.Output.Simple.SavePath")),
	...
	maxTime->setSuffix(" s");
#if QT_VERSION >= QT_VERSION_CHECK(6, 7, 0)
	connect(maxTimeEnable, &QCheckBox::checkStateChanged,
		[this] { maxTime->setEnabled(maxTimeEnable->isChecked()); });
#else
...
#if QT_VERSION >= QT_VERSION_CHECK(6, 7, 0)
	connect(maxSizeEnable, &QCheckBox::checkStateChanged,
		[this] { maxSize->setEnabled(maxSizeEnable->isChecked()); });
#else
	...
	QStringList specList = QString::fromUtf8(obs_frontend_get_locale_string("FilenameFormatting.completer"))
	...
	recordingAdvancedLayout->addRow(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.Settings.Output.Adv.Recording.Filename")),
		filenameFormat);
	...
	recordingAdvancedLayout->addRow(
		QString::fromUtf8(obs_frontend_get_locale_string("Basic.Settings.Output.Format")), fileFormat);
	...
	recordingEncoder->addItem(QString::fromUtf8(obs_frontend_get_locale_string(
	...
	while (obs_enum_encoder_types(idx++, &type)) {
		...
		if ((caps & (OBS_ENCODER_CAP_DEPRECATED | OBS_ENCODER_CAP_INTERNAL)) != 0)
			continue;
		recordingEncoder->addItem(QString::fromUtf8(obs_encoder_get_display_name(type)),
					  QVariant(QString::fromUtf8(type)));
	}
	...
	connect(recordingEncoder, &QComboBox::currentIndexChanged, [recordingAdvancedLayout, this] {
		...
		while (property) {
			AddProperty(property, record_encoder_settings, recordingAdvancedLayout,
...
void OBSBasicSettings::AddServer()
{
	...
	connect(show, &QAbstractButton::toggled, [=](bool hide) {
		show->setText(QString::fromUtf8(hide ? obs_frontend_get_locale_string("Hide")
...
void OBSBasicSettings::SaveSettings()
{
	...
	if (!canvasDock->disable_stream_settings) {
		for (size_t idx = 0; idx < servers.size(); idx++) {
			...
			if (idx >= canvasDock->streamOutputs.size()) {
				...
				so.service = obs_service_create(whip ? "whip_custom" : "rtmp_custom",
								service_name.c_str(), nullptr, nullptr);
	...
	if (streamDelayEnable->isChecked() != canvasDock->stream_delay_enabled ||
	    (canvasDock->stream_delay_enabled &&
	     ((uint32_t)streamDelayDuration->value() != canvasDock->stream_delay_duration ||
		...
		for (auto it = canvasDock->streamOutputs.begin(); it != canvasDock->streamOutputs.end(); ++it) {
			if (!it->output)
				continue;
			obs_output_set_delay(it->output,
					     canvasDock->stream_delay_enabled ? canvasDock->stream_delay_duration : 0,
	...
	for (int i = 1; i <= (int)streamingAudioTracks.size(); i++) {
		if (streamingAudioTracks[i - 1]->isChecked()) {
			if (canvasDock->stream_audio_track != i) {
				for (auto it = canvasDock->streamOutputs.begin(); it != canvasDock->streamOutputs.end();
				     ++it) {
					if (it->output && !obs_output_active(it->output))
	...
	if (canvasDock->record_advanced_settings != ra || canvasDock->record_encoder != re.constData()) {
		...
		if ((canvasDock->recordOutput && !obs_output_active(canvasDock->recordOutput)) ||
			...
			auto enc = canvasDock->recordOutput ? obs_output_get_video_encoder(canvasDock->recordOutput)
							    : nullptr;
		...
		if ((canvasDock->recordOutput && !obs_output_active(canvasDock->recordOutput)) ||
			...
			for (size_t i = 0; i < MAX_AUDIO_MIXES; i++) {
				auto enc = canvasDock->recordOutput
						   ? obs_output_get_audio_encoder(canvasDock->recordOutput, i)
						   : nullptr;
				if (!enc && canvasDock->replayOutput) {
				   ...
				   std::map<obs_property_t *, QWidget *> *widgets)
{
	...
	if (type == OBS_PROPERTY_BOOL) {
		...
		if (text_type == OBS_TEXT_MULTILINE) {
			...
			widget->setPlainText(
				QString::fromUtf8(obs_data_get_string(settings, obs_property_name(property))));
			auto label = new QLabel(QString::fromUtf8(obs_property_description(property)));
			layout->addRow(label, widget);
			if (!obs_property_visible(property)) {
				widget->setVisible(false);
		...
		case OBS_COMBO_FORMAT_INT:
			connect(widget, &QComboBox::currentIndexChanged,
				[this, property, settings, widget, widgets, layout] {
					obs_data_set_int(settings, obs_property_name(property),
	...
	if (type == OBS_PROPERTY_BOOL) {
		...
		if (text_type == OBS_TEXT_MULTILINE) {
			((QPlainTextEdit *)widget)
				->setPlainText(
					QString::fromUtf8(obs_data_get_string(settings, obs_property_name(prop))));
		} else if (text_type != OBS_TEXT_INFO) {
			((QLineEdit *)widget)
				->setText(QString::fromUtf8(obs_data_get_string(settings, obs_property_name(prop))));
		}
	} else if (type == OBS_PROPERTY_LIST) {
